...
class DoubleConvBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, k_size=3, pad=1, dil=1):
        super().__init__()
        # 使用 ReLU 代替 LeakyReLU，与你的 ConvBlock 保持一致
        self.block = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=k_size, padding=pad, dilation=dil, bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_ch, out_ch, kernel_size=k_size, padding=pad, dilation=dil, bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
    def forward(self, x):
...
class Conv3DBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size=3, padding=1, bias=False):
        ...
        self.block = nn.Sequential(
            ...
            nn.BatchNorm3d(out_ch), # 使用 BatchNorm3d
            nn.ReLU(inplace=True)
        )
    def forward(self, x):
        return self.block(x)
...
class carrada(nn.Module):
    def __init__(self, 
        ...
        # self.rd_projector = nn.Conv3d(
        #     in_channels=self.embed_dim,
        #     out_channels=self.embed_dim,
        #     kernel_size=(1, self.grid_size[1], 1), # (1, 32, 1)
        #     stride=(1, 1, 1),
        #     padding=(0, 0, 0)
        # )

        # self.ra_projector = nn.Conv3d(
        #     in_channels=self.embed_dim,
        #     out_channels=self.embed_dim,
        #     kernel_size=(1, 1, self.grid_size[2]), # (1, 1, 8)
        #     stride=(1, 1, 1),
        #     padding=(0, 0, 0)
        # )

        self.rd_projector = nn.Sequential(
            Conv3DBlock(self.embed_dim, self.embed_dim, kernel_size=3, padding=1),
            Conv3DBlock(self.embed_dim, self.embed_dim, kernel_size=3, padding=1),
            # 自适应平均池化，将 W 维度压缩到 1
            nn.AdaptiveAvgPool3d((self.grid_size[0], 1, self.grid_size[2]))
        )

        # RA Projector: 学习聚合特征，然后沿着 D (多普勒/时间) 轴池化
        self.ra_projector = nn.Sequential(
            Conv3DBlock(self.embed_dim, self.embed_dim, kernel_size=3, padding=1),
            Conv3DBlock(self.embed_dim, self.embed_dim, kernel_size=3, padding=1),
            # 自适应平均池化，将 D 维度压缩到 1
            nn.AdaptiveAvgPool3d((self.grid_size[0], self.grid_size[1], 1))
        )

        assert patch_size[0] == patch_size[1] == patch_size[2] and (patch_size[0] & (patch_size[0] - 1) == 0), \
            "All patch sizes must be equal and a power of 2"
        num_stages = int(math.log2(patch_size[0]))


        # 自动计算每层解码器的通道数，逐层减半
        decoder_dims = [self.embed_dim] + [self.embed_dim // (2**i) for i in range(1, num_stages + 1)]
        # 例: embed_dim=192, num_stages=3 -> decoder_dims = [192, 96, 48, 24]

        print(f"解码器阶段数: {num_stages}, 解码器通道维度: {decoder_dims}")

        self.rd_decoder_blocks = nn.ModuleList()
        self.ra_decoder_blocks = nn.ModuleList()
        ...
        for i in range(num_stages):
            ...
            # Range-Doppler 解码器
            rd_up = nn.ConvTranspose2d(in_dim, out_dim, kernel_size=2, stride=2)
            self.rd_decoder_blocks.append(nn.ModuleList([rd_up, DoubleConvBlock(out_dim, out_dim)]))

            # Range-Azimuth 解码器
            ra_up = nn.ConvTranspose2d(in_dim, out_dim, kernel_size=2, stride=2)
            self.ra_decoder_blocks.append(nn.ModuleList([ra_up, DoubleConvBlock(out_dim, out_dim)]))

        final_decoder_dim = decoder_dims[-1]
        self.rd_final = nn.Conv2d(final_decoder_dim, n_classes, kernel_size=1)
        self.ra_final = nn.Conv2d(final_decoder_dim, n_classes, kernel_size=1)
    ...
    def forward(self, x, condition, batch_params):
        ...
        feature = self.forward_feature(x, pos_embed, condition=condition)
        
        x_rd = self.rd_projector(feature).squeeze(3)
        x_rd = torch.flip(x_rd, dims=[2])
        x_ra = self.ra_projector(feature).squeeze(4)


        for rd_block_pair, ra_block_pair in zip(self.rd_decoder_blocks, self.ra_decoder_blocks):
            rd_up, rd_conv = rd_block_pair
            ra_up, ra_conv = ra_block_pair
        ...
        out_rd = self.rd_final(x_rd) 
        out_ra = self.ra_final(x_ra)      

        return out_rd, out_ra # (B, n_classes, R, D), (B, n_classes, R, A)
    
    
def carrada_tiny(**kwargs):
    # 此处 decoder_dims 参数不再需要手动传入，模型内部会自动计算
    model = carrada(
          embed_dim=192,
          depth=12,
          **kwargs,
...
if __name__ == "__main__":
    ...
    #                         测试设置
    # ===================================================================
    print("测试 carrada (Mamba_carrada_tiny) 版本...")
    ...
    print("\n" + "="*80)
    print(f"📋 开始测试 Mamba_carrada_tiny (Batch Size={BATCH_SIZE}, 带FiLM和物理参数)")
    print("="*80)
    ...
    mock_batch_params = [
        ...
        {'range_resolution': torch.tensor(0.115), 'angular_resolution': torch.tensor(0.469), 'time_res': torch.tensor(0.033), 'has_velocity': torch.tensor(False)} # 模拟无速度维度的数据
    ]
    # 手动将字典中的张量堆叠起来，以模拟DataLoader的collate_fn行为
    collated_batch_params = {k: torch.stack([d.get(k, torch.tensor(0.0)) for d in mock_batch_params]) for k in mock_batch_params[0].keys()}
    ...
    try:
        ...
        with torch.no_grad():
            if device.type == 'cuda':
                ...
                start_time.record()
                # 传入所有参数进行推理
                seg_rd, seg_ra = model(x_input, condition=mock_condition_vector, batch_params=collated_batch_params)
                end_time.record()
        ...
        print("\n--- 输出形状 ---")
        print(f"📤 RD Output Shape: {seg_rd.shape} (Expected: ({BATCH_SIZE}, 4, 256, 64))")
        print(f"📤 RA Output Shape: {seg_ra.shape} (Expected: ({BATCH_SIZE}, 4, 256, 256))")
        print(f"⏱️ 推理时间 ({BATCH_SIZE}个样本): {inference_time:.2f}ms")
        print(f"\n✅ Mamba_carrada_tiny 版本测试成功！")
        
        del model, x_input, seg_rd, seg_ra
        if device.type == 'cuda':
            torch.cuda.empty_cache()
            
    except Exception as e:
        print(f"❌ Mamba_carrada_tiny 版本测试失败: {e}")
        import traceback
        traceback.print_exc()
