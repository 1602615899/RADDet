...
if __name__ == '__main__':
    ...
    batch_params_for_model = {
        ...
        'velocity_resolution': torch.tensor([p.get('velocity_resolution', 0.0) for p in dummy_raw_params_list]),
        'time_res': torch.tensor([p.get('time_res', 0.0) for p in dummy_raw_params_list]),
    ...
    # --- 使用新的灵活调用方式进行测试 ---
    autoreg_dims_to_test = ['doppler', 'range', 'angle']

    for dim in autoreg_dims_to_test:
        test_name = f"Tiny (autoreg_dim='{dim}')"
        print(f"\n{'='*20} 测试 VisionMamba-3D {test_name} {'='*20}")
        try:
            # 直接在调用时传入 autoreg_dim 参数
            model = vision_mamba_3d_tiny(autoreg_dim=dim).to(device)
            ...
            print(f"{'='*20} {test_name} 模型测试通过 {'='*20}\n")
        except Exception as e:
            print(f"!!! VisionMamba-3D {test_name} 测试失败: {e}")
            import traceback
            traceback.print_exc()