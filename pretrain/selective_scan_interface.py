...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                ...
                C_proj_bias=None, delta_softplus=True, checkpoint_lvl=1, prompt=None):
        """
             xz: (batch, dim, seqlen)
        """
        ...
        dt_rank_base, B_base, C_base = torch.split(x_dbl, [delta_rank, d_state, d_state], dim=-1)
        if prompt is not None:
            prompt_reshaped = rearrange(prompt, "b l d -> (b l) d")
            prompt_delta, prompt_B, prompt_C = torch.split(
                prompt_reshaped, [delta_rank, d_state, d_state], dim=-1
            )
            dt_rank_base = dt_rank_base + prompt_delta
            B_base = B_base + prompt_B
            C_base = C_base + prompt_C

        delta = rearrange(delta_proj_weight @ dt_rank_base.t(), "d (b l) -> b d l", l=L)
    ...
    def backward(ctx, dout):
        ...
        # 这里改了: 5. 在 backward 的返回值中为新增的非张量输入(prompt, checkpoint_lvl)添加 None 占位符
        return (dxz, dconv1d_weight, dconv1d_bias, dx_proj_weight, ddelta_proj_weight,
                dA, dB, dC, dD,
                ddelta_bias if delta_bias is not None else None,
                dB_proj_bias, dC_proj_bias, None, None, None) # 分别对应 delta_softplus, checkpoint_lvl, prompt
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        ctx.C_proj_bias_is_None = C_proj_bias is None
        if B is None:
            # 这里改了: 3. 使用可能被 prompt 修改过的 B_base
            B = B_base
            if B_proj_bias is not None:
                B = B + B_proj_bias.to(dtype=B.dtype)
            if not A.is_complex():
        ...
        if C is None:
            # 这里改了: 4. 使用可能被 prompt 修改过的 C_base
            C = C_base
            if C_proj_bias is not None:
                C = C + C_proj_bias.to(dtype=C.dtype)
            if not A.is_complex():
...
def mamba_inner_fn(
    ...
    out_proj_weight, out_proj_bias,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True, prompt=None,
):
    return MambaInnerFn.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              out_proj_weight, out_proj_bias,
                              A, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, 1, prompt)
...
def bimamba_inner_fn(
    ...
    return BiMambaInnerFn.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              out_proj_weight, out_proj_bias,
                              A, A_b, B, C, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus, 1, prompt)


def mamba_inner_fn_no_out_proj(
    xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True, prompt=None # <-- 增加 prompt
):
