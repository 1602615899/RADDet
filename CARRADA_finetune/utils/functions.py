...
from models.CARRADA_finetune.losses.LovaszSoftmaxLoss import LovaszSoftmaxLoss
...
def define_loss(signal_type, custom_loss, device, delta = 0.6,  lovasz_weight = 1.0, loss_weight = 1., dice_weight = 10., coherence_weight = 5.):
    """
    ...
    if custom_loss == 'wce':
        ...
        weights = get_class_weights(signal_type)
        ce_loss = CALoss(global_weight=loss_weight, delta= delta, device = device)
        lovasz_loss = LovaszSoftmaxLoss(global_weight=dice_weight, classes='all',per_image=False,ignore_index=None)
        loss = [ce_loss, lovasz_loss, MVLoss(global_weight=coherence_weight)]
...
def get_qualitatives(outputs, masks, paths, seq_name, quali_iter, signal_type=None):
    ...
    quali_iter: int
    """
    base_path = Path(paths)
    if signal_type:
        folder_path = base_path / signal_type / seq_name[0]
    else:
        folder_path = base_path / seq_name[0]
    ...
    for i in range(outputs.shape[0]):
        ...
        mask_path = folder_path / 'mask_{}.png'.format(quali_iter[0])
        mask_img.save(mask_path, dpi=(300, 300))
        output_img = mask_to_img(outputs[i])
        output_path = folder_path / 'output_{}.png'.format(quali_iter[0])
